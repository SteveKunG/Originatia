...
public class WarpSelectionScreen extends Screen
{
    ...
    public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks)
    {
        if (!this.isKeyPressed())
        {
            ...
            for (WarpSelectionScreen.SelectorWidget widget : this.gameModeButtons)
            {
    ...
    {
        if (!InputMappings.isKeyDown(this.minecraft.getMainWindow().getHandle(), GLFW.GLFW_KEY_LEFT_CONTROL))
        {
            this.sendMessage();
    ...
    {
        if (keyCode == KeyBindingHandler.KEY_QUICK_NAVIGATOR.getKey().getKeyCode())
        {
    ...
    enum Mode
    {
        ...
        private Mode(ITextComponent displayName, String command, ItemStack displayItem)
        {
        ...
        {
            itemRenderer.renderItemAndEffectIntoGUI(this.displayItem, x, y);
        }
    ...
    public class SelectorWidget extends Widget
    {
        ...
        private void renderUnselected(MatrixStack matrixStack, TextureManager manager)
        {
            manager.bindTexture(WarpSelectionScreen.TEXTURE);
            matrixStack.push();
        ...
        private void renderSelected(MatrixStack matrixStack, TextureManager manager)
        {
            manager.bindTexture(WarpSelectionScreen.TEXTURE);
            matrixStack.push();
            matrixStack.translate(this.x, this.y, 0.0D);
